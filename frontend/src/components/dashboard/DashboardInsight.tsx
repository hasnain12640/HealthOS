import { Bot, Link2, MessageCircle, ShieldAlert } from 'lucide-react'
...
const domainBadgeStyles: Record<InsightDomain, string> = {
  ...
  sleep: 'text-[#A78BFA] bg-[#A78BFA]/10 border-[#A78BFA]/20',
    ...
    <section className="dashboard-glass overflow-hidden rounded-3xl border border-primary/25 p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <p className="dashboard-eyebrow">{t['insight.title']}</p>
            <h2 className="mt-1 text-lg font-semibold text-text-primary">{t['dashboard.ai_intelligence']}</h2>
            <div className="mt-1 flex items-center gap-2 text-[10px] text-text-muted">
              <span>{insightData?.date ?? fallbackInsight.date}</span>
              {insightData && (
                <span className={[
      ...
      ) : insightData?.text || fallbackInsight.text ? (
        <div className="mt-5">
          <p className="text-sm leading-relaxed text-text-primary">{insightData?.text ?? fallbackInsight.text}</p>
          <p className="mt-3 flex items-start gap-1.5 text-xs italic text-text-muted"><ShieldAlert size={13} className="mt-0.5 shrink-0" />{t['dashboard.ai_disclaimer']}</p>
        </div>
      ) : (
