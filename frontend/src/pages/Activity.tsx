      ...
      <div className="space-y-4">
        ...
        <Card>
          <h3 className="text-text-primary font-semibold text-sm mb-3 flex items-center gap-2">
            <Moon size={14} className="text-primary-light" /> Sleep (Last 7 Days)
          </h3>
          <div className="flex items-center gap-6 mb-3">
            <div>
              <p className="text-text-primary font-bold text-2xl">{sleep.avg_hours}h</p>
              <p className="text-text-secondary text-xs">7-day average</p>
            </div>
            <div>
          ...
          <div className="flex items-end gap-1 h-16">
            {sleep.logs.map((s, i) => {
              ...
              const color = h >= 7 ? 'var(--color-status-normal)' : h >= 6 ? 'var(--color-status-low)' : 'var(--color-status-high)'
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t" style={{ height: `${pct}%`, backgroundColor: color, minHeight: '4px' }} />
                  <span className="text-text-muted text-[10px]">{h}h</span>
                </div>
              )
            })}
          </div>
        ...
        <Card>
          <h3 className="text-text-primary font-semibold text-sm mb-3 flex items-center gap-2">
            <Activity size={14} className="text-accent" /> Weekly Activity
          </h3>
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-text-primary font-bold text-xl">{activity.total_minutes} min</p>
              <p className="text-text-secondary text-xs">of {activity.weekly_target_min} min target this week</p>
            </div>
          ...
          <div className="h-2 bg-bg-elevated rounded-full overflow-hidden mb-4">
          ...
          <div className="space-y-2">
            {activity.recent.map(a => (
              <div key={a.id} className="flex items-center justify-between bg-bg-elevated rounded-lg px-3 py-2">
                <div>
                  <p className="text-text-primary text-sm capitalize">{a.activity_type}</p>
                  <p className="text-text-muted text-xs">{a.date}</p>
                </div>
                <div className="text-right">
                  <p className="text-text-primary text-sm">{a.duration_min} min</p>
                  <p className="text-text-muted text-xs">{a.steps.toLocaleString()} steps</p>
                </div>
              </div>
            ))}
          </div>
