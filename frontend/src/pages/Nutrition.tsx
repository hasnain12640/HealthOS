...
export function Nutrition() {
  ...
  const macros = [
    { label: 'Calories', value: `${data.total_calories} kcal`, target: `Target: ${data.calorie_target} kcal`, pct: data.calorie_percent, color: '#F59E0B' },
    { label: 'Protein', value: `${data.total_protein_g}g`, target: `Target: ${data.protein_target_g}g`, pct: data.protein_percent, color: '#0EA5E9' },
    { label: 'Carbohydrates', value: `${data.total_carbs_g}g`, target: '', pct: null, color: '#10B981' },
    { label: 'Fat', value: `${data.total_fat_g}g`, target: '', pct: null, color: '#EF4444' },
  ]

  const calStatusColor = data.calorie_status === 'good' ? '#10B981' : data.calorie_status === 'low' ? '#F59E0B' : '#EF4444'
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {macros.map(({ label, value, target, color }) => (
            <Card key={label} padding="md">
              ...
              {target && <p className="text-[#6B7280] text-xs mt-0.5">{target}</p>}
            </Card>
          ))}
        </div>
        ...
        <Card>
          <h3 className="text-[#F9FAFB] font-semibold text-sm mb-3 flex items-center gap-2">
            <Apple size={14} className="text-[#10B981]" /> Today's Meals ({data.meal_count})
          </h3>
          <div className="space-y-2">
            {data.meals_today.map(m => (
              <div key={m.id} className="flex items-center justify-between bg-[#1F2937] rounded-lg px-3 py-2">
                <div>
                  <p className="text-[#F9FAFB] text-sm">{m.food_name}</p>
                  <p className="text-[#6B7280] text-xs capitalize">
                    {m.meal_type}{m.is_pakistani_food ? ' · Pakistani' : ''}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[#F9FAFB] text-sm font-medium">{m.calories} kcal</p>
                  <p className="text-[#6B7280] text-xs">{m.protein_g}g protein</p>
                </div>
              </div>
            ))}
          </div>
